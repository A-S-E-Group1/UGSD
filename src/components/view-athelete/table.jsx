...
const ViewAthleteTable = ({ data }) => {
	...
	const [athlete, setAtlete] = useState(null);
	...
	function sumMedalCounts(medals) {
		let total = 0;

		// Loop through each organization or event
		for (const organization in medals) {
			const organizationMedals = medals[organization];

			// Loop through each medal type
			for (const medalType in organizationMedals) {
				const count = organizationMedals[medalType];

				// Convert count to a number and add it to the total if it's a valid number
							...
							{data?.map((athlete, index) => {
								return (
									<Tr key={index}>
										...
										<Td>
											<Text>{sumMedalCounts(athlete.medals)}</Text>
										</Td>
										<Td>
